...
分布式系统中，组件间通信常用的方式是消息发布订阅机制。即配置一个配置共享中心，提供者在配置中心发布消息，使用者则订阅他们关心的主题，一旦有关主题有消息发布，就会实时通知订阅者。可通过watch机制实现。应用启动时，主动从etcd获取一次配置信息，同时在etcd节点注册一个watcher，以后每次配置有更新，etcd都会实时通知订阅者，从而及时获取最新配置信息。


### 分布式锁

etcd支持revision机制，那么对于同一个lock，即便有多个客户端争夺，revision机制可保证它们的revision编号有序切唯一，那么，客户端只要根据revision大小顺序就可以确定获取锁的先后顺序，从而实现公平锁。

### 集群监控与leader竞选 

通过watch机制，当某个key消失或变动时，watcher会第一时间发现并告知用户。节点可为key设置租约，比如每隔30s向etcd发送一次心跳，代表该节点的key保持存活，一旦节点故障，续约停止，对应的key将失效删除。如此通过watch机制可第一时间检测到各节点健康状态，从而完成集群监控。

使用分布式锁可很好实现leader竞选（抢锁成功的为leader）。leader应用的经典场景是在搜索系统中建立全量索引。如果每个机器分别进行索引的建立，不仅耗时，而且不能保证一致性。通过在etcd实现锁机制竞选leader，由leader进行索引计算，再将结果分发到其他节点。
...
- 互斥性：在任意时刻，对于同一个锁，只能有一个客户端持有；
- 安全性：不会形成死锁，当一个客户端在持有锁期间崩溃而未能主动解锁的情况下，其持有的锁也能够被正确释放，并保证后续其他客户端能枷锁；
- 可用性：当提供锁服务的节点发生宕机等不可恢复性故障时，热备节点能够接替故障节点继续提供服务，并保持自身持有的数据与故障节点一致；
-对称性：对任意一个锁，其加锁和解锁必须是同一个客户端，即，a不能把b的锁给解了；

lease机制，即租约机制。etcd可为kv设置租约，当ttl到期，kv将失效删除，同时也支持续约，通过客户端可在租约到期之前续约，避免kv过期失效。lease机制可保证分布式锁的安全性，为锁对应的key配置租约，即使锁的持有者因故障而不能主动释放锁，锁也会因租约到期而自动释放。

revision机制。每个key带有一个revision号，每进行一次事务加一，因此它是全局唯一的。通过revision大小就可知道进行写操作的顺序。在实现分布式锁时，多个客户端同时抢锁，根据revision大小一次获得锁，可避免羊群效应，实现公平锁。

prefix机制，即前缀机制，也称目录机制。一个名为/lock的锁，两个争抢的客户端进行写操作，实际写入的key分别为：k1=/lock/uuid1,k2=/lock/uuid2。写操作都会成功，但返回的revision不一样，通过查询/lock返回包含两个kv的列表，同时包含各自的revision，通过revision大小，客户端可以判断自己是否获得锁，如果抢锁失败，则等待锁释放（对应的key被删除或者租约过期），然后再判断自己是否可以获得锁。

watch机制，即监听机制。支持监听某个固定的key，也支持一个目录，当被监听的key或目录发生变化，client将收到通知。在实现分布式锁时，如果抢锁失败，则可通过prefix机制返回的kv列表获得revision比自己小切相差最小的pre-key，对pre-key进行监听，因为只有它释放锁，自己才能所得，如果watch到pre-key的delete事件，则说明prek-key以释放，自己获得锁。
